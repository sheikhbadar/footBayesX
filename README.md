...
### Avoid Data Leakage
Always use **lagged** rolling statistics. Computing a 5-match rolling average without lagging includes the current match in the calculation, which leaks future information into the model. Use `lag()` in R when building rolling features.

### Standardise on Training Data Only
Always compute `scale()` on the training set and apply the same `center` and `scale` attributes to the test set. Scaling the full dataset before splitting leaks test distribution information into training.
