...
- `K` = number of user-supplied covariates

---

## One Covariate Vector Per Match — Not Two

A key design decision: footBayesX does **not** require separate covariate matrices for home and away teams.

You supply **one row per match**, not one row per team per match:

```
X[n, ] = [ feature_1_diff,  feature_2_diff,  ...,  feature_K_diff ]
```

Each feature is constructed as a **(home value − away value)** difference **before** passing to Stan.

**Example** with K = 2 features (form difference, Elo difference):
...
This single row encodes who has the advantage on each feature:

| X[n, k] value | Meaning |
|---|---|
| `> 0` | Home team has the advantage on feature k |
| `< 0` | Away team has the advantage on feature k |
| `= 0` | Teams are equal on feature k |

Because the sign already encodes direction, the same row can drive home scoring up and away scoring down simultaneously — using `+Xβ` for home and `−Xβ` for away.

---

## Why +Xβ for Home and −Xβ for Away

The symmetric sign design is not a convention. It is a **statistical identifiability requirement**.

### The zero-sum constraint

The model enforces a sum-to-zero constraint on team effects:
...
This means the model learns only from **relative differences** between teams, not absolute levels. Without this constraint, U could add any constant to all attacking parameters and subtract it from the intercept — the likelihood would be identical and the model would have infinitely many solutions.

### Why X must follow the same antisymmetric structure

The model's output is determined by the log-ratio of home to away scoring:

```
log(θ_home) − log(θ_away) = 2 × X[n]β  +  team ability terms
```

This relative quantity is what identifies β. Now consider what happens with both signs positive:

```
log(θ_home) = ... + Xβ
log(θ_away) = ... + Xβ   ← same sign

log(θ_home) − log(θ_away) = 0 contribution from X
```

**X disappears from the relative comparison entirely.** Instead of encoding which team has the advantage, it becomes a global match-level intensity inflator — shifting both teams' expected goals up or down together. This effect is already captured by the team `att` and `def` parameters, meaning β cannot be separately identified from the existing model structure.

### The connection to footBayes ranking term

This is exactly the same logic already used in the original footBayes model for the ranking term:
...
footBayesX generalises this principle: the ranking difference is replaced by any user-supplied (home − away) difference feature, using the same antisymmetric sign structure.

### Summary

| Design | Effect on log(θ_home) − log(θ_away) | Identifiable? |
|---|---|---|
| `+Xβ` / `−Xβ` (this model) | `+2Xβ` — X explains relative advantage |  Yes |
| `+Xβ` / `+Xβ` (both positive) | `0` — X cancels out in comparison |  No |

> **One-line rule:** X is a home-minus-away difference, so β must tilt the scoreline in opposite directions for the two teams. The plus/minus signs enforce this.
...
- The away team scores `exp(−0.25) ≈ 0.78×` fewer goals

### Covariate Categories Users Can Build

All covariates must be constructed as **(home value − away value)** before passing to Stan.

#### Category 1: Form and Performance Differences

| Covariates | How to construct in R |
|---|---|
...
1. **Always use lagged covariates** — compute rolling statistics using only past matches to avoid data leakage
2. **Always standardise using training data** — `scale(X_train)` then apply the same centre and scale to `X_test`
3. **Covariates should be differences** — absolute values (e.g. home stadium capacity alone) are not compatible with the `+Xβ / −Xβ` design
...
## B) Wrapper Function: stan_foot_multi()
...
# Example: Using Multiple Covariates
...
Where:
- `X` is an N × K design matrix of covariates  
...
We can visually analyse the attack and defense effects using the `foot_abilities` function:

```r
foot_abilities(fit_multi, italy_df)
```
...
`X` must contain **(home − away) difference covariates**. The model uses `+Xβ` for home intensity and `−Xβ` for away intensity. This means a positive value in `X[n, k]` is interpreted as a home-team advantage on feature k. Absolute features (e.g. home stadium capacity without subtracting away capacity) are not compatible with this design.

### One Row Per Match

`X` has **one row per match**, not one row per team. Each row encodes the relative advantage of the home team over the away team across all K features. This is why only one coefficient vector `β` is needed — the direction of advantage is already encoded in the sign of each `X[n, k]` value.

### Global β Assumption

`β` is a single global coefficient vector estimated across all N matches. This assumes the effect of each covariate is **stationary**  the same momentum advantage has the same expected goal impact in match 1 as in match N. This is a deliberate parsimony assumption. If you believe covariate effects vary over time or by league strength, a time-varying `β[t]` extension would be needed.

This assumption is testable: if posterior `Rhat ≈ 1.00` and `ess_bulk > 400` for all `β[k]`, the global assumption is consistent with the data.
...
```r
devtools::install_github("sheikhbadar/footBayesX")
```
...
- Multi-covariate football modelling
- Bayesian inference on match-level covariates
- LOO / WAIC model comparison
- Extension of footBayes without modifying the original package
